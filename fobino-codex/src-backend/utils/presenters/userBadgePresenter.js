...
function mapProducerLevel(producer, user = {}) {
...
function mapProducerStatus(level, producer, user = {}) {
...
function buildBadges({ user, subscription, producer }) {
  ...
  if (activeSubscription?.plan === 'producer') {
    const level = mapProducerLevel(producer, user);
    badges.producer = {
      active: true,
      verificationLevel: level,
      stars: [0, 1, 2].map((index) => index < level),
      label: buildProducerLabel(level, true),
      status: mapProducerStatus(level, producer, user),
      expiresAt: activeSubscription.endDate || null,
    };
  }
...
async function buildBadgesForUsers(usersOrIds = []) {
  ...
  const now = new Date();
  const [subscriptions, producers] = await Promise.all([
    Subscription.find({
      user: { $in: ids },
